...
Let's try this out: suppose we have sample data of 
\[2,2,3,7,8,9\]

In R, we can save this as a vector, then make it a data frame that we can manipulate with `dplyr`:

```{r data_ex1}
sample_data <- data.frame(c(2,2,3,7,8,9))
...
```{marginfigure}
Note the use of `round` in the last command; here we are rounding to 15 digits. If you just use `mean()` in the last step, you will get a very small number in scientific notation. The actual value of the mean is in fact 0, but you are seeing a very small number due to a *floating point error* in R. These are very common in any programming language, and dealing with them is a necessary evil.
```

You should find that the mean deviation is 0! The positive deviations are cancelled out by the negative deviations. This will always happen in any data set, since the average value of the data points is 0 units away from the average. Therefore, this is not a very useful measurement of spread. So what can we do?